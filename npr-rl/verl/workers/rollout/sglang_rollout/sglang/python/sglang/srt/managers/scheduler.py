...
class Scheduler(
    ...
    def __init__(
        ...
        if not self.is_generation:
            ...
            logger.debug("Overlap scheduler is disabled for embedding models.")
    ...
    def handle_generate_request(
        ...
        """Manage generate req and Add to waitting queue"""
        ...
        if (
            ...
            recv_req.sampling_params.repetition_penalty = 1.0  # recover
            req = Req(
                recv_req.rid,
                recv_req.input_text,
                recv_req.input_ids,
    ...
    def check_memory(self):
        ...
        if self.tree_cache.evictable_size() > 0:
            evictable_indices = []
            def collect_evictable(node):
                # node: TreeNode
                # If node.lock_ref == 0, node.value is evictable
                if node.lock_ref == 0 and len(node.value) > 0:
                    # node.value maybe torch tensor or list
                    v = node.value
                    if isinstance(v, torch.Tensor):
                        evictable_indices.extend(v.cpu().numpy().tolist())
                    else:
                        evictable_indices.extend(list(v))
                for child in getattr(node, "children", {}).values():
                    collect_evictable(child)

            collect_evictable(self.tree_cache.root_node)
            evict_set = set(evictable_indices)
            evictable_size = len(evict_set)

            available_size = self.token_to_kv_pool_allocator.available_size() + evictable_size
    ...
    def get_next_batch_to_run(self) -> Optional[ScheduleBatch]:
        """Step 3: Create ScheduleBatch for req"""
        ...
        if self.last_batch and self.last_batch.forward_mode.is_extend():
            ...
            if not self.last_batch.is_empty():
                ...
                if self.zombie_batch is not None:
                    self.running_batch = self.last_batch  # for proccess zombie_batch
            ...
            if not self.running_batch.is_empty():
                ...
                if self.running_batch.is_zombie and (not self.running_batch.is_merge): # Split
                    if len(self.running_batch.pending_reqs) > 0:
                        ...
                        # patch for zombie state but no pending reqs
    ...
    def split_merge_zombie_batch_to_run(self, zombie_group: List[Tuple[str, List[Req]]]):
        ...
        for _, item in enumerate(zombie_group):
            ...
            reqs[0].sampling_params.repetition_penalty = 1.0  # recover
            new_req = Req(
                parent_rid,
    ...
    def run_batch(
        ...
        """
        Run a batch.
        Step 4: Convert ScheduleBatch to ModelWorkerBatch
        """
        self.forward_ct += 1
    ...
    def set_internal_state(self, recv_req: SetInternalStateReq):
        ...
        if if_success:
            if not self.spec_algorithm.is_none() and self.cum_spec_accept_count > 0:
                ...
                logger.debug(f"{avg_spec_accept_length=}")
            self.cum_spec_accept_length = self.cum_spec_accept_count = 0
            for k, v in server_args_dict.items():
                global_server_args_dict[k] = v
            logger.debug(f"Global server args updated! " f"{global_server_args_dict=}")
