...
class Req:
    ...
    def check_finished(self):
        ...
        if self.right_most_pos - self.init_input_len >= self.sampling_params.max_new_tokens - 128:
        ...
        if self.sampling_params.stop_token_ids:
            if (len(self.origin_input_ids) - self.init_input_len) + (len(self.output_ids) * self.parallel_degree) \
                    >= self.sampling_params.max_new_tokens - 128:
        ...
        if not self.sampling_params.ignore_eos:
            ...
            if self.sampling_params.stop_token_ids:
                ...
                import warnings
                warnings.warn("Please set stop_token_ids!")
...
class ScheduleBatch(ScheduleBatchDisaggregationDecodeMixin):
    ...
    def prepare_for_extend(self):
        ...
        parallel_degree = [r.parallel_degree for r in reqs]
    ...
    def prepare_for_decode(self):
        ...
        self.parallel_degree = [r.parallel_degree for r in self.reqs]
    ...
    def filter_batch(
        ...
        for idx in range(len(fork_indices)):
            ...
            # hyperparameter 1024 is the minimal remained tokens for split
            if dead_req.right_most_pos - dead_req.init_input_len >= dead_req.sampling_params.max_new_tokens - 1024 \
        ...
        for key, value in self.p2c_map.items():
            ...
            if len(values) == 0:  # every path is finished
                v = value.copy()
                zombie_group_item = (key, [])
                # in order & double check
                for rid in v:
                    for req in self.zombie_reqs:
                        if req.rid == rid:
...
class ModelWorkerBatch:
    ...
    parallel_degree: Optional[List[int]]
