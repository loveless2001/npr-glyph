...
from verl.trainer.ppo.core_algos import agg_loss, compute_policy_loss, get_policy_loss_fn, kl_penalty, compute_policy_loss_archer
...
from verl.workers.actor.mv_utils import generate_parallel_attention_mask
...
class DataParallelPPOActor(BasePPOActor):
        ...
        with torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            ...
            if self.use_remove_padding:
                ...
                for i in range(batch_size):
                    ...
                    nonpad = (input_ids[i] != self.tokenizer.pad_token_id).nonzero(as_tuple=True)[0]
                    start = nonpad[0].item()
                    end = nonpad[-1].item() + 1
                    mv_input_ids = input_ids[i][start: end]
                    mv_position_ids = position_ids[i][:len(mv_input_ids)]

                    mv_attention_mask, _ = generate_parallel_attention_mask(mv_input_ids, self.tokenizer)

                    mv_input_ids = mv_input_ids.unsqueeze(0)
                    mv_position_ids = mv_position_ids.unsqueeze(0)
                    mv_attention_mask = mv_attention_mask.unsqueeze(0).unsqueeze(1).cuda()

                    # too slow
                    logits_item = self.actor_module(
                        input_ids=mv_input_ids,
                        attention_mask=mv_attention_mask,
                        position_ids=mv_position_ids,
                        **multi_modal_inputs,
                        use_cache=False,
                        **extra_args,
                    ).logits

                    logits_item = F.pad(logits_item, (0, 0, start, seqlen - end), value=0)
                    logits_list.append(logits_item)
    ...
    def update_policy(self, data: DataProto):
        ...
        on_policy = self.config.ppo_mini_batch_size == 1 and self.config.ppo_epochs == 1
        on_policy = True
        metrics = {}
        for _ in range(self.config.ppo_epochs):
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    if self.config.policy_loss.loss_mode == "aspo":
                        calculate_entropy = True
                    ...
                    elif self.config.policy_loss.loss_mode == "aspo":
