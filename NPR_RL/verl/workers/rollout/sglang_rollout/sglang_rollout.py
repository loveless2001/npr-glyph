...
from verl.workers.actor.mv_utils import generate_parallel_attention_mask, generate_parallel_position_ids
...
def _post_process_outputs_mv(tokenizer, output, idx_list, record_steps, exp_name, n_, tp_rank=0):

    idx = 0
    max_length = 30000  # TODO_wt: hyperparameter
    keep_indices = []
    batched_output_token_ids, batched_position_ids = [], []

    if tp_rank == 0:
        af_long = open(f"experiments/record/{exp_name}/over_long_case.txt", "a")
        af_bad = open(f"experiments/record/{exp_name}/step{record_steps}.txt", "a")

    for idx_item in tqdm(idx_list):
        for i in range(n_):

            output_token_ids = tokenizer.encode(output[idx]["text"], return_tensors="pt", add_special_tokens=False).squeeze(0)

            mv_attention_mask, mask_is_bad_case = generate_parallel_attention_mask(output_token_ids, tokenizer)
            mv_position_ids, pos_is_bad_case = generate_parallel_position_ids(output_token_ids, tokenizer)

            if mask_is_bad_case or pos_is_bad_case:
                keep_indices.append(0)
                batched_position_ids.append(torch.arange(len(idx_item)))
                if tp_rank == 0:
                    af_bad.write(output[idx]["text"])
                    af_bad.write("\n" + "=" * 50 + "\n")
            elif len(output_token_ids[len(idx_item):]) > max_length:
                keep_indices.append(0)
                batched_position_ids.append(torch.arange(len(idx_item)))
                if tp_rank == 0:
                    af_long.write(output[idx]["text"])
                    af_long.write("\n" + "=" * 50 + "\n")
            else:
                keep_indices.append(1)
                batched_position_ids.append(mv_position_ids)

            batched_output_token_ids.append(output_token_ids[len(idx_item):])
            idx += 1

    if tp_rank == 0:
        af_long.close()
        af_bad.close()
        print('=' * 15, f"{sum(keep_indices)}/{len(idx_list) * n_}", '=' * 15)

    pad_token_id = tokenizer.pad_token_id if tokenizer.pad_token_id is not None else tokenizer.eos_token_id
    batched_output_token_ids = pad_sequence(batched_output_token_ids, batch_first=True, padding_value=pad_token_id)
    batched_position_ids = pad_sequence(batched_position_ids, batch_first=True, padding_value=pad_token_id)

    return batched_output_token_ids, batched_position_ids, keep_indices
...
_worker_lock = None
_worker_tp_rank = 0
_worker_exp_name = None
_worker_record_steps = None
_worker_max_length = 30000


def _worker_init(tokenizer, lock, tp_rank, exp_name, record_steps, max_length):
    """Simple initializer for child processes."""
    global _worker_tokenizer, _worker_lock, _worker_tp_rank, _worker_exp_name, _worker_record_steps, _worker_max_length
    _worker_tokenizer = tokenizer
    _worker_lock = lock
    _worker_tp_rank = tp_rank
    _worker_exp_name = exp_name
    _worker_record_steps = record_steps
    _worker_max_length = max_length


def _worker(task):
    """Process one (idx_item, output_text, original_index) task and return serializable result."""
    idx_item, output_text = task

    token_ids = _worker_tokenizer.encode(output_text, return_tensors='pt', add_special_tokens=False).squeeze(0)

    mv_mask, mask_bad = generate_parallel_attention_mask(token_ids, _worker_tokenizer)
    mv_pos, pos_bad = generate_parallel_position_ids(token_ids, _worker_tokenizer)

    if mask_bad or pos_bad:
        keep = 0
        pos_list = list(range(len(idx_item)))
        if _worker_tp_rank == 0:
            with _worker_lock:
                p = os.path.join('experiments', 'record', _worker_exp_name, f'step{_worker_record_steps}.txt')
                with open(p, 'a', encoding='utf-8') as f:
                    f.write(output_text + '\n' + '=' * 50 + '\n')
    elif len(token_ids[len(idx_item):]) > _worker_max_length:
        keep = 0
        pos_list = list(range(len(idx_item)))
        if _worker_tp_rank == 0:
            with _worker_lock:
                p = os.path.join('experiments', 'record', _worker_exp_name, 'over_long_case.txt')
                with open(p,'a',encoding='utf-8') as f:
                    f.write(output_text + '\n' + '=' * 50 + '\n')
    else:
        keep = 1
        pos_list = mv_pos.tolist()

    sliced = token_ids[len(idx_item):].tolist()
    return (sliced, pos_list, keep)


def _post_process_outputs_mv_simple(tokenizer, output, idx_list, record_steps, exp_name, n_, tp_rank=0, nprocs=4, max_length=30000):
    """Minimal multiprocessing implementation that preserves order.

    Returns: batched_output_token_ids, batched_position_ids, keep_indices
    """
    ...
    for idx_item in idx_list:
        for _ in range(n_):
            tasks.append((idx_item, output[idx]['text']))
            idx += 1
    ...
    with ctx.Pool(processes=nprocs, initializer=_worker_init, initargs=(tokenizer, lock, tp_rank, exp_name, record_steps, max_length)) as pool:
        # map preserves order -> results correspond to tasks order
        results = list(tqdm(pool.map(_worker, tasks), total=len(tasks)))

    # unpack
    sliced_token_lists = [r[0] for r in results]
    position_lists = [r[1] for r in results]
    keep_indices = [r[2] for r in results]

    pad_token_id = tokenizer.pad_token_id if tokenizer.pad_token_id is not None else tokenizer.eos_token_id
    tensor_list = [torch.tensor(x, dtype=torch.long) for x in sliced_token_lists]
    pos_tensor_list = [torch.tensor(x,dtype=torch.long) for x in position_lists]

    batched_output_token_ids = pad_sequence(tensor_list, batch_first=True, padding_value=pad_token_id)
    batched_position_ids = pad_sequence(pos_tensor_list, batch_first=True, padding_value=pad_token_id)

    if tp_rank == 0:
        print('='*15, f"{sum(keep_indices)}/{len(tasks)}", '='*15)
...
class SGLangRollout(BaseRollout):
    ...
    def _batch_level_generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        sample_batch_size = min(len(idx_list), 32)  # TODO_wt: hyperparameter
        idx_list = [idx_list[i: i + sample_batch_size] for i in range(0, len(idx_list), sample_batch_size)]
        ...
        try:
            ...
            try:
                ...
                raise ValueError(f"Cannot get tokenizer from processing_class {self.processing_class}") from e
        ...
        if self._tp_rank == 0:
            ...
            for idx_item in idx_list:
                loop = asyncio.get_event_loop()
                output_item = loop.run_until_complete(
                    self._engine.async_generate(
                        prompt=None,  # because we have already convert it to prompt token id
                        sampling_params=request_sampling_params,
                        input_ids=idx_item,
        ...
        if is_validate:
            ...
            for i, idx_item in enumerate(idx_list):
                output_token_ids = tokenizer.encode(output[i]["text"], return_tensors="pt", add_special_tokens=False).squeeze(0)
                batched_output_token_ids.append(output_token_ids[len(idx_item):].squeeze(0))
            pad_token_id = tokenizer.pad_token_id if tokenizer.pad_token_id is not None else tokenizer.eos_token_id
            out = pad_sequence(batched_output_token_ids, batch_first=True, padding_value=pad_token_id)
        else:
            n_ = self.sampling_params.get("n", 1)
            exp_name = self.config.exp_name
            record_steps = self.config.record_steps
            out, pos_id, keep_indices = _post_process_outputs_mv_simple(
                tokenizer, output, idx_list, record_steps, exp_name, n_, self._tp_rank)
            self.config.record_steps += 1

        # out = _post_process_outputs(self.processing_class, output)
        # response = out[0].to(idx.device)
        response = out.to(idx.device).to(idx.dtype)
        if not is_validate:
            position_ids = pos_id.to(idx.device).to(idx.dtype)
        ...
        if response.shape[1] > self.config.response_length:
            logger.warning(f"response length {response.shape[1]} is greater than response_length {self.config.response_length}, truncating")
        ...
        if position_ids.shape[1] > (idx.shape[1] + self.config.response_length):
            position_ids = position_ids[:, : idx.shape[1] + self.config.response_length]
        if position_ids.shape[1] < (idx.shape[1] + self.config.response_length):
            position_ids = pad_sequence_to_length(position_ids, idx.shape[1] + self.config.response_length, self.pad_token_id)

        # if not is_validate:
        #     batch_size = response.size(0) // self._tp_size * self._tp_size
        #     idx = idx[keep_indices][:batch_size]
        #     attention_mask = attention_mask[keep_indices][:batch_size]
        ...
        # print(idx.shape, response.shape, seq.shape, attention_mask.shape, position_ids.shape, torch.tensor(keep_indices).shape)
        ...
        batch = TensorDict(
            {
                ...
                "position_ids": position_ids if not is_validate else None,
                "keep_indices": torch.tensor(keep_indices) if not is_validate else None,
            },
