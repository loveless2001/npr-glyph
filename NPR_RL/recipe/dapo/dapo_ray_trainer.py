...
class RayDAPOTrainer(RayPPOTrainer):
    ...
    def fit(self):
        ...
        if not os.path.exists(f"experiments/record/{self.config.trainer.experiment_name}"):
            os.makedirs(f"experiments/record/{self.config.trainer.experiment_name}")
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                #if self.gen_steps < 25:
                #    print(f"{self.gen_steps=}")
                ...
                with marked_timer("step", timing_raw):
                    ...
                    if not self.config.algorithm.filter_groups.enable:
                        batch = new_batch
                        true_traj_bsz = batch.batch.batch_size[0] // 8 * 8  # TODO_wt: hyperparameter
                        print(f"{true_traj_bsz=} v.s. {batch.batch.batch_size[0]=}")
                        ...
                        print(f"{len(prompt_uid2metric_std)=}")
                        print(f"{num_prompt_in_batch=}")
                    ...
                    on_policy = (self.config.actor_rollout_ref.actor.ppo_mini_batch_size == 1 \
                                 and self.config.actor_rollout_ref.actor.ppo_epochs == 1)
                    on_policy = True
                    if not on_policy:
                ...
                if reward_extra_infos_dict is not None:
                    for key, sequence_extra in reward_extra_infos_dict.items():
                        metrics.update({
