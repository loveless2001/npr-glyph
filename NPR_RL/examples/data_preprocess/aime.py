...
"""
    ...
    test_dataset = datasets.load_dataset("math-ai/aime25")["test"]
    aime25 = test_dataset.map(function=make_map_fn(data_source), with_indices=True)
    print(aime25)
    print(aime25[0])


    data_source = "aime2024"
    test_dataset = datasets.load_dataset("HuggingFaceH4/aime_2024")["test"]
    aime24 = test_dataset.map(function=make_map_fn(data_source), with_indices=True)
    print(aime24)
    print(aime24[0])


    test_dataset = datasets.concatenate_datasets([aime24, aime25])
    test_dataset.to_parquet(os.path.join(args.local_dir, "aime.parquet"))
